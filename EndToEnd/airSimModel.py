...
from environment.manager import AirSimCar
import Cooking
...
class AirSimModel(BaseModel):
    ...
    def __init__(self,config):
        ...
        # Data generators
        self.trainDataGenerator = None
        self. evalDataGenerator = None
    ...
    def train(self):
        # Read data
        trainData = h5py.File(os.path.join(self.config.dir_cooked,"train.h5"),'r')
        evalData  = h5py.File(os.path.join(self.config.dir_cooked, "eval.h5"),'r')
        
        self.config.n_train = trainData['image'].shape[0]
        self.config.n_eval  =  evalData['image'].shape[0]

        batch_size = self.config.batch_size
        generator = DriveDataGenerator(rescale=1./255.,horizontal_flip=True,brighten_range=0.4)

        trainDataGenerator = generator.flow(trainData['image'],trainData['previous_state'],trainData['label'], 
                                            batch_size=batch_size, zero_drop_percentage=0.95, roi=[76,135,0,255])
        evalDataGenerator  = generator.flow( evalData['image'], evalData['previous_state'], evalData['label'], 
                                            batch_size=batch_size, zero_drop_percentage=0.95, roi=[76,135,0,255])
        
        return self.model.train(trainDataGenerator,evalDataGenerator)


    def test(self):
        # Read data
        testData = h5py.File(os.path.join(self.config.dir_cooked, "test.h5"),'r')     
        self.config.n_test = testData['image'].shape[0]
    ...
    def play(self):
        ...
        while (True):
            ...
            model_output = self.model.predict([image_buf, state_buf])
