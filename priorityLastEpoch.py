    ...
    def load(self,path):
        ...
        with open(path, 'rb') as handle:
            ...
            if 'priority' in db:
                ...
                limit         = db['priority'].limit
                priority      = db['priority']._data[n_leaf-1:limit+n_leaf-1] # <----- Check limit+n_leaf-1

            if 'UTC' in db:
                sampleCounter = db['sampleCounter']
                n_samples     = sampleCounter.shape[0]
                UTC           = db[     'UTC']._data[n_leaf-1:limit+n_leaf-1] # <----- Check limit+n_leaf-1
